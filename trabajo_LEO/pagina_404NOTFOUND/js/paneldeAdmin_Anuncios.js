document.addEventListener('DOMContentLoaded', () => {
  ...
  const cambiarEstado = async (id, estado) => {
    try {
      ...
      mostrarNotificacion(`Anuncio ${estado.toLowerCase()} correctamente`, 'exito');
      cargarAnuncios(); // Recargar toda la tabla para asegurar consistencia
    } catch (error) {
      console.error('Error:', error);
      mostrarNotificacion(`Error al ${estado.toLowerCase()} el anuncio`, 'error');
    }
  };
  ...
  const cargarAnuncios = async () => {
    try {
      ...
      if (response.data.length === 0) {
        tablaBody.innerHTML = '<tr><td colspan="6">No hay anuncios disponibles</td></tr>';
        return;
      }
      ...
      response.data.forEach(anuncio => {
        ...
        fila.innerHTML = `
          ...
          <td>${anuncio.autor || 'Anónimo'}</td>
          <td>${new Date(anuncio.fecha || anuncio.createdAt).toLocaleDateString()}</td>
          <td class="estado">${anuncio.estado || 'Pendiente'}</td>
          <td>
            <button class="btn aceptar" data-id="${anuncio._id}">Aprobar</button>
            <button class="btn rechazar" data-id="${anuncio._id}">Rechazar</button>
            <button class="btn eliminar" data-id="${anuncio._id}">Eliminar</button>
          </td>`;
  ...
  const asignarEventosBotones = () => {
    document.querySelectorAll('.aceptar').forEach(btn => {
      btn.addEventListener('click', (e) => {
        cambiarEstado(e.target.dataset.id, 'Aprobado');
      });
    });
    
    document.querySelectorAll('.rechazar').forEach(btn => {
      btn.addEventListener('click', (e) => {
        cambiarEstado(e.target.dataset.id, 'Rechazado');
      });
    });
  ...
  document.getElementById('recargar-anuncios')?.addEventListener('click', cargarAnuncios);
