...
async function cargarPendientes() {
  ...
  try {
    const data = await fetchJSON(`${API_BASE}/api/admin/reporteSugerencias/pendientes`);
    ...
    tbody.innerHTML = items.map(x => `
      <tr data-id="${x._id}">
        ...
        <td data-label="Usuario">${x.usuarioId ?? '-'}</td>
        <td data-label="Tipo">${x.categoria ?? '-'}</td>
        <td data-label="Mensaje">${x.contenido ?? '-'}</td>
...
async function aprobar(id, btn) {
  setBusyRow(btn, true);
  try {
    await fetchJSON(`${API_BASE}/api/admin/reporteSugerencias/${id}/aprobar`, { method: 'PATCH' });
...
async function rechazar(id, btn) {
  setBusyRow(btn, true);
  try {
    await fetchJSON(`${API_BASE}/api/admin/reporteSugerencias/${id}/rechazar`, { method: 'PATCH' });
    const tr = document.querySelector(`tr[data-id="${id}"]`);
    if (tr) tr.remove();
    updateCounter();
    const tbody = document.getElementById('tbody-pendientes');
